...
- (void)viewDidLoad {
  ...
  for (int i=0; i<10; i++) {
    ...
    [self.feedDataArray addObject:dict];
  }
}
...
-(void)registerCells{
  ...
  [self.tableView registerNib:nib forCellReuseIdentifier:@"SectionHeaderOfPost"];
  
  nib = [UINib nibWithNibName:@"UploadedOn" bundle:nil];
  [self.tableView registerNib:nib forCellReuseIdentifier:@"UploadedOn"];
  
  nib = [UINib nibWithNibName:@"TotalLikes_Comments" bundle:nil];
  [self.tableView registerNib:nib forCellReuseIdentifier:@"TotalLikes_Comments"];
  
  nib = [UINib nibWithNibName:@"PostImage" bundle:nil];
  [self.tableView registerNib:nib forCellReuseIdentifier:@"PostImage"];
  
  nib = [UINib nibWithNibName:@"Buttons" bundle:nil];
  [self.tableView registerNib:nib forCellReuseIdentifier:@"Buttons"];
  
  nib = [UINib nibWithNibName:@"DetailText" bundle:nil];
  [self.tableView registerNib:nib forCellReuseIdentifier:@"DetailText"];
  
  nib = [UINib nibWithNibName:@"PostDescription" bundle:nil];
  [self.tableView registerNib:nib forCellReuseIdentifier:@"PostDescription"];
  
  nib = [UINib nibWithNibName:@"SuggestedPeopleCell" bundle:nil];
  [self.tableView registerNib:nib forCellReuseIdentifier:@"SuggestedPeopleCell"];
}
...
-(NSInteger)numberOfSectionsInTableView:(UITableView *)tableView{
  return self.feedDataArray.count+1;
}
...
- (NSInteger)tableView:(UITableView *)aTableView numberOfRowsInSection:(NSInteger)section {
  if (section==0) {
    ...
    NSArray * arr =[[self.feedDataArray objectAtIndex:section-1]valueForKey:@"dataToshow"];
    return arr.count;
...
- (UITableViewCell *)tableView:(UITableView *)aTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if (indexPath.section==0) {
    ...
    __block  NSMutableDictionary * dataDict =[[NSMutableDictionary alloc]initWithDictionary:[self.feedDataArray objectAtIndex:indexPath.section-1]];
